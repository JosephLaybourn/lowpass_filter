...
class LowPassResDemo : public Control {
  ...
  private:
    ...
    bool stereo;
    float rate;
    LowPassRes2 lp2L, lp2R;
    LowPassRes3 lp3L, lp3R;
    float gain;
    int order;
};
...
LowPassResDemo::LowPassResDemo(float R, short *s, unsigned n, bool st)
    : Control(4,"Resonant Low Pass Filter Demo"),
      samples(s),
      count(n),
      index(0),
      stereo(st),
      rate(R),
      lp2L(500,0,R), lp2R(500,0,R),
      lp3L(500,0,R), lp3R(500,0,R),
      gain(1),
      order(2) {
  ...
  // 2: order
  setRange(2,2,3);      // range: {2,3}
  setValue(2,2);        // default: 2
  // 3: gain
  setRange(3,0,1000);   // range: -24dB - 24dB
  setValue(3,500);      // default: 0dB
}
  ...
  switch (index) {
    case 0: // frequency
      {  float frequency = 5*pow(10,1+0.002f*value);
         lp2L.setFrequency(frequency);  lp2R.setFrequency(frequency);
         lp3L.setFrequency(frequency);  lp3R.setFrequency(frequency);
    ...
    case 1: // resonance
      { float r = 0.001f*value;
        lp2L.setResonance(r);  lp2R.setResonance(r);
        lp3L.setResonance(r);  lp3R.setResonance(r);
        ss << "Resonance: " << fixed << setprecision(3) << r;
      }
      break;
    case 2: // filter order
      order = value;
      ss << "Roll-off: " << (value*6) << "dB/oct";
      break;
    case 3: // gain
...
int LowPassResDemo::OnWrite(const void *vin, void *vout, unsigned long frames,
  ...
  if (demo.stereo) {
    for (unsigned long i=0; i < frames; ++i) {
      ...
      L = (demo.order == 2) ? demo.lp2L(L) : demo.lp3L(L);
      R = (demo.order == 2) ? demo.lp2L(R) : demo.lp3R(R);
      out[2*i] = demo.gain * L;
    ...
    for (unsigned long i=0; i < frames; ++i) {
      ...
      x = (demo.order == 2) ? demo.lp2L(x) : demo.lp3L(x);
      out[i] = demo.gain * x;
      demo.index = (demo.index + 1) % demo.count;
    }
  }
